...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Dialogs, Menus, BaseUnix,
  SynHighlighterPas, SynEdit, LCLType, ComCtrls, ExtCtrls, Process,
  SynGutterLineNumber, SynEditTypes, SynHighlighterCpp;
...
type
  ...
  TScrol = class(TForm)
    ...
    procedure CHighlightMenuItemClick(Sender: TObject);
...
begin
    ...
    Caption := 'Scroll';
      ...
      { Comments: medium forest green }
        ...
        { Preprocessor directives: amber/gold }
        SynCppSyn1.DirecAttri.Foreground      := 26265;     { #996600 }
        SynCppSyn1.DirecAttri.ExtendPastEol   := True;

        { Identifiers: editor default }
        SynCppSyn1.IdentifierAttri.Foreground := clDefault;

        { Keywords: medium blue }
        SynCppSyn1.KeyAttri.Foreground        := 12277026;  { #2255BB }

        { Numbers: medium purple }
        SynCppSyn1.NumberAttri.Foreground     := 11154295;  { #7733AA }

        { Strings: brick red }
        SynCppSyn1.StringAttri.Foreground     := 2241467;   { #BB3322 }
        SynCppSyn1.StringAttri.ExtendPastEol  := False;

        { Symbols: editor default }
        SynCppSyn1.SymbolAttri.Foreground     := clDefault;
...
procedure TScrol.CHighlightMenuItemClick(Sender: TObject);
begin
  ...
  if Sender is TMenuItem then
    (Sender as TMenuItem).Checked := (SynEdit1.Highlighter = SynCppSyn1);
end;
